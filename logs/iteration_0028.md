# Hadamard Max-Det Research — Iteration 0028

**Timestamp:** 2026-04-28 10:13:19
**Target:** Order 23 ±1 matrix, max determinant
**Best Known:** 2,779,447,296,000,000
**Barba Bound:** 3,919,726,327,358,822

---

## Generated Candidates

1. `gs_paley_baseline` (strategy_hermod_goethals_seidel)
2. `gs_block_flip` (strategy_hermod_goethals_seidel)
3. `gs_anti_paley` (strategy_hermod_goethals_seidel)
4. `gs_mixed` (strategy_hermod_goethals_seidel)
5. `gs_toeplitz_skew` (strategy_hermod_goethals_seidel)

---

## Test Results

| # | Name | |det| | % Barba | Method |
|---|------|--------|---------|--------|
| 1 | `gs_paley_baseline` | 1,521,681,143,169,024 | 38.8211% | strategy_hermod_goethals_seidel |
| 2 | `gs_anti_paley` | 1,521,681,143,169,024 | 38.8211% | strategy_hermod_goethals_seidel |
| 3 | `gs_mixed` | 1,521,681,143,169,024 | 38.8211% | strategy_hermod_goethals_seidel |
| 4 | `gs_block_flip` | 186,934,156,591,104 | 4.7691% | strategy_hermod_goethals_seidel |
| 5 | `gs_toeplitz_skew` | 134,934,953,984 | 0.0034% | strategy_hermod_goethals_seidel |

---

## Analysis

**Top determinant:** 1,521,681,143,169,024 (38.8211% of Barba)
  Construction: strategy_hermod_goethals_seidel
  Improves record? No

**Method performance this iteration:**
  strategy_hermod_goethals_seidel: best=1,521,681,143,169,024 ↑ (was 0)
  Spread: 1,521,546,208,215,040 (max-min)

---

## Self-Assessment

**Overall Progress (Iteration 28)**
  Best determinant:    1,521,681,143,169,035
  vs. best known:      2,779,447,296,000,000 (54.75%)
  vs. Barba bound:     3,919,726,327,358,822 (38.8211%)
  Total tested:        140

**Method Rankings (by best determinant):**
  hadamard_submatrix: 1,521,681,143,169,035 (17 attempts)
  strategy_seed_conference_border: 1,521,681,143,169,024 (10 attempts)
  strategy_hermod_diagonal_paley: 1,521,681,143,169,024 (10 attempts)
  strategy_hermod_diffset_construction: 1,521,681,143,169,024 (10 attempts)
  strategy_hermod_goethals_seidel: 1,521,681,143,169,024 (5 attempts)

---

## Next Plan

**Strategy: BASELINE** — gathering initial data across methods
  Continue diverse exploration

**Next iteration candidate sources:**
  1. Continue current top method with perturbations
  2. Try unexplored methods: negacyclic, skew-symmetric

---

## State Summary

Best determinant found: 1,521,681,143,169,035
  vs. best known:   2,779,447,296,000,000
  Barba bound:      3,919,726,327,358,822 (38.82%)
  Construction:      hadamard_submatrix
  Iterations:        28
  Total tested:      140
